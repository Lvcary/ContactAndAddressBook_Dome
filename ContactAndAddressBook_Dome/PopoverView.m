...
-(CGRect)getViewFrame
{
    ...
    CGFloat realHeigh = kScreenHeight - self.showPoint.y - SPACE - kArrowHeight;
    if (frame.size.height > realHeigh)
    {
        frame.size.height = realHeigh;
    }
    
    frame.size.width = kScreenWidth * 0.5 + 20;
...
//    }
    ...
    //右间隔最小5x
    if ((frame.origin.x + frame.size.width) > (kScreenWidth - frame.origin.x))
    {
        frame.origin.x = kScreenWidth - frame.size.width - 5;
    }
...
-(UITableView *)tableView
{
    ...
    rect.size.height -= (SPACE + kArrowHeight + 0.5);
